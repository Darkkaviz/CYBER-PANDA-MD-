...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
 ...
 switch (type) {
     ...
     conn.sendFile(m.chat, img, 'logo.png', `👸 *BY QUEEN-SAKURA-MD*`, m)
     m.react(done)
     break 
     case 'neon': 
     if (!text) throw tee
     let ne = global.API('fgmods', '/api/textpro/neon', { text }, 'apikey')
     conn.sendFile(m.chat, ne, 'logo.png', `👸 *BY QUEEN-SAKURA-MD*`, m)
     m.react(done)
     break 
     case 'devil': 
     if (!text) throw tee
     let de = global.API('fgmods', '/api/textpro/devil', { text }, 'apikey')
     conn.sendFile(m.chat, de, 'logo.png', `👸 *BY QUEEN-SAKURA-MD*`, m)
     m.react(done)
     break 
     case 'wolf': 
    if (!text) throw tee
    let wo = global.API('fgmods', '/api/textpro/logowolf', { text: 'SAKURA-MD', text2: text}, 'apikey')
     conn.sendFile(m.chat, wo, 'logo.png', `👸 *BY QUEEN-SAKURA-MD*`, m)
     m.react(done)
     break 
     case 'phlogo': 
     if (!text) throw too
     ...
     conn.sendFile(m.chat, ph, 'logo.png', `👸 *QUEEN-SAKURA-MD*`, m)
     m.react(done)
     break 
     default:
 } 
